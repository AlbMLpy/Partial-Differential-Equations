...
class SDE:
    ...
    def _newman_cond(
        ...
        if (i == self.n - 1) and (j == self.n - 1):
            if mode == 1:
                self.fg[self.n*j + i] = 2 * self.g_n(x_i, y_j) / self.h
            ...
            if mode == 2:
                self.fg[self.n*j + i] = (
                    -0.5 * (self.B * self.h * self.g_n(x_i, y_j)
                            + self.f(x_i, y_j, self.eps))
                )
    ...
    def plot_err(self):
        ...
        z = (self.x_appr - self.x_true).reshape(self.n, self.n)
        fig, ax = plt.subplots()
        cs = ax.contourf(
            z,
            levels = lev_region,
